...
- [X] T005 [P] 予定→ログ初期値の pure mapper を新設 in `service-front/src/features/dives/lib/planToDiveDefaults/planToDiveDefaults.ts`（`planToDiveDefaults(plan: PlanView): Partial<DiveFormValues>` = `{ diveDate: plan.plannedOn, location: plan.location, notes: plan.notes ?? undefined }`。`diveSiteId` は設定しない）+ 再export `index.ts`。contracts/ui-and-forms.md §3 / data-model.md マッピング表準拠
- [X] T006 [P] T005 の Vitest を作成 in `service-front/src/features/dives/lib/planToDiveDefaults/planToDiveDefaults.test.ts`: plannedOn→diveDate・location→location・notes→notes（null 時は undefined）・`diveSiteId` 未設定・`maxDepthM`/`bottomTimeMin` を含まないことを検証。**FR-008 は design-covered**（plan/dive とも location 120・notes 2000 で上限一致のため切り詰め不要）である旨をコメントで残し、上限一致を前提とした 1 ケース（上限長の notes がそのまま引き継がれる）を含める
