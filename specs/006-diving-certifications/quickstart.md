...
> SC-001 の確認: 手順 2 の登録 1 件が 1 分以内に完了することを体感確認する
...
6. 任意項目（ダイバーナンバー・インストラクターナンバー・指導者/ショップ名・取得場所）とスペシャリティタグを入力して別の資格を登録 → 一覧に入力済みの項目とタグのみが表示される（FR-011 / FR-012）
7. 「取得ダイブ」に自分のダイビングログを選択して登録 → 一覧に該当ダイブログへのリンクが表示される（FR-014）
