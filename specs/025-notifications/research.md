...
- **Decision**: 遷移先は表示時に種別から静的に組み立てる（`followed` → プロフィール URL（034 以降は actor のニックネーム URL・未取得時は ID フォールバック）、`buddy_tagged` → `/dives/[resource_id]`、`plan_reminder` → `/plans/[resource_id]`、`overhaul_reminder` → `/settings/equipment`）。対象が消えている場合は各遷移先ページの既存 404 / フォールバック（`notFound()` や一覧リダイレクト）に委ね、通知一覧側では事前の存在チェックをしない。
