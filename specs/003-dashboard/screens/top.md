...
| ID | 要素 | 種別 | 内容 |
|----|------|------|------|
| E1 | ヒーロー | テキスト + ボタン | ユーザー名 / 経過日数 / 「＋ 新しいログを記録」 / 「保有資格を管理」（outline。006 で追加） |
...
| E6 | （オプション）月別グラフ | チャート | 🟡 TBD: Phase 1 で入れるか後回しか → **003-dashboard では対象外（後続 feature で検討）** |
| E7 | 残枠バッジ | `CreditBalanceBadge`（リンク） | `残りログ枠 N`。ページ上部右寄せ。`/settings/log-credits`（購入ページ）への導線を兼ねる（026 で追加） |
...
| 保有資格管理 | E1 の「保有資格を管理」 | `/settings/certifications`（[006 保有資格管理](../../006-diving-certifications/spec.md) で追加） |
