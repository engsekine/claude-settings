...
| # | id | title | 主な内容 | 例示表示（候補） |
|---|---|---|---|---|
| 1 | `getting-started` | はじめに | 登録 → プロフィール設定 → 最初のログ作成 → ダッシュボード確認の基本フロー | なし（テキストのみ） |
| 2 | `dive-logs` | ダイブログを記録する | ログの作成・編集・写真添付・検索 | ログカード（サンプル） |
| 3 | `plans-packing` | ダイビング予定と持ち物リスト | 予定作成・持ち物チェック・予定からログへの記録 | 予定カード（サンプル） |
| 4 | `dashboard` | ダッシュボードで振り返る | 統計・推移チャート・レギュレーター OH の見方 | 統計カード（サンプル） |
| 5 | `social-likes` | みんなのログ・いいね | タイムライン閲覧・いいね・いいねしたログ一覧 | なし（テキストのみ） |
| 6 | `log-credits` | ログ枠と追加購入 | 作成上限（ログ枠）の仕組みと残枠確認・追加購入の流れ | なし（テキストのみ） |

- 例示表示の最終選定は実装時に表示専用性を確認して行う（操作系・Server Action 付きは不可）
