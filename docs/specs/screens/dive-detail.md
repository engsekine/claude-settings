...
1 本のダイブログの全項目を閲覧する画面。編集・削除の起点となる。所有者以外は RLS により 404 となるため、本画面に到達した時点で表示中のログは閲覧者自身のもの。
...
| コンディション | 流れ | `dives.current` | 文字列 | 行ごと非表示 |
...
|------|------|------|
| 通常 | 自分のログ | 全項目表示 |
| 404 | 他人のログ / 存在しない id | Next.js の `notFound()` で 404 ページ |
...
- 認証済みユーザーのみ閲覧可能
- 自分のログのみ閲覧可能（RLS で保証、他人の id は 404）
...
- 実装: `service-front/src/app/(app)/dives/[id]/page.tsx`
